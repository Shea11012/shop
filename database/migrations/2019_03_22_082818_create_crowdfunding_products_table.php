...
class CreateCrowdfundingProductsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('crowdfunding_products', function (Blueprint $table) {
            ...
            $table->decimal('total_amount',10,2)->comment('当前已筹金额');
            $table->unsignedInteger('user_count')->comment('当前参与众筹用户数');
            $table->dateTime('end_at')->comment('众筹结束时间');
            $table->string('status')->comment('当前筹款状态')->default(\App\Models\CrowdfundingProduct::STATUS_FUNDING);
        });
    }
